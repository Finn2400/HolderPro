...
def tracked(root: Path) -> list[str]:
    ...
    result = [item.decode("utf-8", errors="surrogateescape") for item in raw.split(b"\0") if item]
    if not result:
        raise RuntimeError("repository has no tracked files; corresponding source must come from a commit")
    return sorted(result)


def forbidden(path: str, patterns: list[str]) -> bool:
    return any(Path(path).match(pattern) or fnmatch.fnmatchcase(path, pattern) for pattern in patterns)
...
def validate_dependency_manifest(
    ...
    if document.get("holderpro_build_id") != holderpro_commit:
        raise RuntimeError("dependency manifest is bound to a different HolderPro commit")
    ...
    required_names = {
        item["name"] for item in requirements.get("components", [])
    }
    ...
    for item in components:
        if not isinstance(item, dict) or not item.get("name") or not item.get("version") or not item.get("license"):
            raise RuntimeError(f"incomplete dependency manifest entry: {item!r}")
        if not item.get("source_commit") and not (item.get("source_url") and item.get("source_sha256")):
            raise RuntimeError(f"dependency has no verifiable source locator: {item.get('name')}")
        if item.get("ecosystem") == "pypi" and item.get("release_constraint") != f"=={item.get('version')}":
            raise RuntimeError(
...
def write_manifest(root: Path) -> None:
    ...
    for path in sorted(root.rglob("*")):
        if path.is_file() and not path.is_symlink() and path.name != "SOURCE-MANIFEST.sha256":
...
def main() -> int:
    ...
    if not (prusa / PRUSA_REQUIRED).is_file():
        raise SystemExit(f"PrusaSlicer checkout is incomplete: missing {PRUSA_REQUIRED}")
    ...
    source_rules = tomllib.loads((repository / "packaging/source-manifest.toml").read_text(encoding="utf-8"))
    ...
    bad = [
        ...
        and not any(path == item or path.startswith(item + "/") for item in fixture_roots)
    ]
    if bad:
        raise SystemExit("forbidden tracked source paths:\n- " + "\n- ".join(bad))

    epoch_text = os.environ.get("SOURCE_DATE_EPOCH")
    epoch = int(epoch_text) if epoch_text else int(run("git", "show", "-s", "--format=%ct", "HEAD", cwd=repository))
    ...
    with tempfile.TemporaryDirectory(prefix="holderpro-source-") as temporary:
        ...
        if output_path.name.endswith(".tar.zst"):
            subprocess.run(["zstd", "-q", "-19", "-f", str(raw_tar), "-o", str(output_path)], check=True)
